...
go
--  Viết thủ tục hàm doanh thu
CREATE PROC doanhThuNV
		@SalesPerson int,
		@SalesYTD int output 
AS
BEGIN
	-- Gọi biến temp và Thoigian (Thời gian đầu năm bằng cách ghép chuỗi)
	DECLARE @temp varchar(10), @Thoigian datetime 
	set @temp = convert(varchar(12), YEAR(getdate())) + '-01-01'
	set @Thoigian = convert(datetime, @temp)
	set @SalesYTD = (
			select doanhThu = sum([SubTotal])
			from [Sales].[SalesOrderHeader] soh
			where soh.SalesOrderID = @SalesPerson and  [OrderDate] between @Thoigian and getdate()
		)
	-- Trả về giá trị cho thủ tục 
		IF(@SalesYTD = NULL)
			RETURN -1
		ELSE IF @SalesYTD > 0
			RETURN @SalesYTD
...
-- Tạo biến nhân viên và kết quả cần output
DECLARE	@result int, @maNV int
set @maNV = 278
set @result = 0

-- Nhập bằng bàn phím maNV và result(kết quả) cần truyền vào
EXEC doanhThuNV @maNV, @result output

-- In ra bảng messages
IF(@result > 0)
	PRINT N'Nhân viên có mã: '+ convert(varchar(4), @maNV)+ N' có tổng doanh thu từ đầu năm đến hiện nay là: '+convert(varchar(10), @result)
ELSE 
	PRINT N'Nhân viên có mã: '+ convert(varchar(4), @maNV)+ N' không có doanh thu từ đầu năm đến bây giờ là: 0'

DROP PROC doanhThuNV
